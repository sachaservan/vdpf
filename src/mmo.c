...
// does one round of MMO where H_i+1 = AES(H_i, m_i) XOR m_i
uint128_t* mmoRound(EVP_CIPHER_CTX* mmoCtx, uint128_t *prevdigest, uint128_t *message){

    uint128_t input = *prevdigest;
    input ^= (*message);

    int len = 0;
    uint128_t output = 0;
    if(1 != EVP_EncryptUpdate(mmoCtx, (uint8_t*)prevdigest, &len, (uint8_t*)&input, 16))
